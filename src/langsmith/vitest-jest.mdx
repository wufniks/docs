...
* creates a [dataset](/langsmith/evaluation-overview#datasets) with the same name as the name passed to `ls.describe()` in LangSmith if it does not exist
...
* creates a new [experiment](/langsmith/evaluation-overview#experiment) with one result for each test case
