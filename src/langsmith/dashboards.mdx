...
| :------ | :------------ |
| Traces | Trace count, latency and error rates. A [trace](/langsmith/observability-overview#traces) is a collection of [runs](/langsmith/observability-overview#runs) related to a single operation. For example, if a user request triggers an agent, all runs for that agent invocation would be part of the same trace. |
