...
Some commonly-used endpoints and use cases are listed below. For a complete list of available endpoints, see the [API docs](https://api.smith.langchain.com/redoc). **The `X-Organization-Id` header should be present on all requests, and either the `X-Tenant-Id` or `X-Tenant-Name` header should be present on requests that are scoped to a particular workspace.**
...
<Note>
Use either the `X-Tenant-Id` (Workspace ID) or `X-Tenant-Name` (Workspace Name) header to specify which workspace to target. If the header is not present, operations will default to the workspace the API key was initially created in.
</Note>
