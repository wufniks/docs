...
<Check>
  For more information on datasets, evaluations and examples, read the [concepts guide on evaluation and datasets](/langsmith/evaluation-overview#datasets-and-examples).
</Check>
...
A common pattern for constructing datasets is to convert notable traces from your application into dataset examples. This approach requires that you have [tracing to LangSmith configured](/langsmith/observability-overview#tracing-configuration).
...
You can use [run rules](/langsmith/rules) to automatically add traces to a dataset based on certain conditions. For example, you could add all traces that are [tagged](/langsmith/observability-overview#tags) with a specific use case or have a [low feedback score](/langsmith/observability-overview#feedback).
...
On the [**Prompt Playground**](/langsmith/observability-overview#prompt-playground) page, select **Set up Evaluation**, click **+New** if you're starting a new dataset or select from an existing dataset.
