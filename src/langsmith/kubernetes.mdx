...
1. A working Kubernetes cluster that you can access via `kubectl`. Your cluster should have the following minimum requirements:
   ...
   2. Valid Dynamic PV provisioner or PVs available on your cluster.
      ...
      * On EKS, you may need to ensure you have the `ebs-csi-driver` installed and configured for dynamic provisioning. Refer to the [EBS CSI Driver documentation](https://docs.aws.amazon.com/eks/latest/userguide/ebs-csi.html) for more information.
      ...
      ```
        NAME            PROVISIONER                 RECLAIMPOLICY   VOLUMEBINDINGMODE      ALLOWVOLUMEEXPANSION   AGE  gp2 (default)   ebs.csi.eks.amazonaws.com   Delete          WaitForFirstConsumer   true                   161d
      ```
      ...
      Refer to the [Kubernetes documentation](https://kubernetes.io/do/langsmith/observability-overview/storage/storage-classes/) for more information on storage classes.
...
4. Run `helm install langsmith langchain/langsmith --values langsmith_config.yaml --version <version> -n <namespace> --wait --debug`
