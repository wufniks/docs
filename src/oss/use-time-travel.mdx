...
2. [Identify a checkpoint in an existing thread](#2-identify-a-checkpoint): Use the @[`get_state_history()`][get_state_history] method to retrieve the execution history for a specific `thread_id` and locate the desired `checkpoint_id`.  
  Alternatively, set an [interrupt](/oss/add-human-in-the-loop) before the node(s) where you want execution to pause. You can then find the most recent checkpoint recorded up to that interrupt.
...
<Tip>
  For a conceptual overview of time-travel, see [Time travel](/oss/time-travel).
</Tip>
