...
Here we show how to define and update [state](/oss/graph-api#state) in LangGraph. We will demonstrate:

1. How to use state to define a graph's [schema](/oss/graph-api#schema)
2. How to use [reducers](/oss/graph-api#reducers) to control how state updates are processed.

### Define state

:::python
[State](/oss/graph-api#state) in LangGraph can be a `TypedDict`, `Pydantic` model, or dataclass. Below we will use `TypedDict`. See [this section](#use-pydantic-models-for-graph-state) for detail on using Pydantic.
:::

:::js
[State](/oss/graph-api#state) in LangGraph can be defined using Zod schemas. Below we will use Zod. See [this section](#alternative-state-definitions) for detail on using alternative approaches.
:::
...
Let's consider a simple example using [messages](/oss/graph-api#messagesstate). This represents a versatile formulation of state for many LLM applications. See our [concepts page](/oss/graph-api#working-with-messages-in-graph-state) for more detail.
...
:::python
Let's build an example graph with a single node. Our [node](/oss/graph-api#nodes) is just a Python function that reads our graph's state and makes updates to it. The first argument to this function will always be the state:
...
:::js
Let's build an example graph with a single node. Our [node](/oss/graph-api#nodes) is just a TypeScript function that reads our graph's state and makes updates to it. The first argument to this function will always be the state:
...
:::python
Let's next define a simple graph containing this node. We use [StateGraph](/oss/graph-api#stategraph) to define a graph that operates on this state. We then use [add_node](/oss/graph-api#nodes) populate our graph.
...
:::js
Let's next define a simple graph containing this node. We use [StateGraph](/oss/graph-api#stategraph) to define a graph that operates on this state. We then use [addNode](/oss/graph-api#nodes) populate our graph.
...
Each key in the state can have its own independent [reducer](/oss/graph-api#reducers) function, which controls how updates from nodes are applied. If no reducer function is explicitly specified then it is assumed that all updates to the key should override it.
...
* We may want to accept short-hands for [message formats](/oss/graph-api#using-messages-in-your-graph), such as [OpenAI format](https://python.langchain.com/docs/concepts/messages/#openai-format).
...
:::python
To add a sequence of nodes, we use the `.add_node` and `.add_edge` methods of our [graph](/oss/graph-api#stategraph):
...
:::js
To add a sequence of nodes, we use the `.addNode` and `.addEdge` methods of our [graph](/oss/graph-api#stategraph):
...
* How state updates are [checkpointed](/oss/persistence)
* How interruptions are resumed in [human-in-the-loop](/oss/human-in-the-loop) workflows
* How we can "rewind" and branch-off executions using LangGraph's [time travel](/oss/time-travel) features

They also determine how execution steps are [streamed](/oss/streaming), and how your application is visualized
...
Let's first define our [state](/oss/graph-api#state). This governs the [schema of the graph](/oss/graph-api#schema), and can also specify how to apply updates. See [this section](#process-state-updates-with-reducers) for more detail.
...
:::python
Our [nodes](/oss/graph-api#nodes) are just Python functions that read our graph's state and make updates to it. The first argument to this function will always be the state:
...
:::js
Our [nodes](/oss/graph-api#nodes) are just TypeScript functions that read our graph's state and make updates to it. The first argument to this function will always be the state:
...
<Note>
  ...
  By default, this will **overwrite** the value of the corresponding key. You can also use [reducers](/oss/graph-api#reducers) to control how updates are processed— for example, you can append successive updates to a key instead. See [this section](#process-state-updates-with-reducers) for more detail.
</Note>

Finally, we define the graph. We use [StateGraph](/oss/graph-api#stategraph) to define a graph that operates on this state.

:::python
We will then use [add_node](/oss/graph-api#messagesstate) and [add_edge](/oss/graph-api#edges) to populate our graph and define its control flow.
...
:::js
We will then use [addNode](/oss/graph-api#nodes) and [addEdge](/oss/graph-api#edges) to populate our graph and define its control flow.
...
* We must specify the entry point of the graph. For this we add an edge with the [START node](/oss/graph-api#start-node).
* The graph halts when there are no more nodes to execute.

We next [compile](/oss/graph-api#compiling-your-graph) our graph. This provides a few basic checks on the structure of the graph (e.g., identifying orphaned nodes). If we were adding persistence to our application via a [checkpointer](/oss/persistence), it would also be passed in here.
...
In this example, we fan out from `Node A` to `B and C` and then fan in to `D`. With our state, [we specify the reducer add operation](/oss/graph-api#reducers). This will combine or accumulate values for the specific key in the State, rather than simply overwriting the existing value. For lists, this means concatenating the new list with the existing list. See the above section on [state reducers](#process-state-updates-with-reducers) for more detail on updating state with reducers.
...
<Note>
  In the above example, nodes `"b"` and `"c"` are executed concurrently in the same [superstep](/oss/graph-api#graphs). Because they are in the same step, node `"d"` executes after both `"b"` and `"c"` are finished.
  
  Importantly, updates from a parallel superstep may not be ordered consistently. If you need a consistent, predetermined ordering of updates from a parallel superstep, you should write the outputs to a separate field in the state together with a value with which to order them.
</Note>

<Accordion title="Exception handling?">
  LangGraph executes nodes within [supersteps](/oss/graph-api#graphs), meaning that while parallel branches are executed in parallel, the entire superstep is **transactional**. If any of these branches raises an exception, **none** of the updates are applied to the state (the entire superstep errors).
  
  Importantly, when using a [checkpointer](/oss/persistence), results from successful nodes within a superstep are saved, and don't repeat when resumed.
...
When creating a graph with a loop, we require a mechanism for terminating execution. This is most commonly done by adding a [conditional edge](/oss/graph-api#conditional-edges) that routes to the [END](/oss/graph-api#end-node) node once we reach some termination condition.

You can also set the graph recursion limit when invoking or streaming the graph. The recursion limit sets the number of [supersteps](/oss/graph-api#graphs) that the graph is allowed to execute before it raises an error. Read more about the concept of recursion limits [here](/oss/graph-api#recursion-limit).
...
This architecture is similar to a [ReAct agent](/oss/agentic-architectures) in which node `"a"` is a tool-calling model, and node `"b"` represents the tools.
...
In some applications, we may not have a guarantee that we will reach a given termination condition. In these cases, we can set the graph's [recursion limit](/oss/graph-api#recursion-limit). This will raise a `GraphRecursionError` after a given number of [supersteps](/oss/graph-api#graphs). We can then catch and handle this exception:
...
<Accordion title="Extended example: loops with branches">
  ...
  This graph looks complex, but can be conceptualized as loop of [supersteps](/oss/graph-api#graphs):
...
<Tip>
  **Async streaming**
  See the [streaming guide](/oss/streaming) for examples of streaming with async.
</Tip>
:::
...
We can now create the `StateGraph` with the above nodes. Notice that the graph doesn't have [conditional edges](/oss/graph-api#conditional-edges) for routing! This is because control flow is defined with `Command` inside `node_a`.
...
We can now create the `StateGraph` with the above nodes. Notice that the graph doesn't have [conditional edges](/oss/graph-api#conditional-edges) for routing! This is because control flow is defined with `Command` inside `nodeA`.
...
If you are using [subgraphs](/oss/subgraphs), you might want to navigate from a node within a subgraph to a different subgraph (i.e. a different node in the parent graph). To do so, you can specify `graph=Command.PARENT` in `Command`:
...
<Warning>
  **State updates with `Command.PARENT`**
  When you send updates from a subgraph node to a parent graph node for a key that's shared by both parent and subgraph [state schemas](/oss/graph-api#schema), you **must** define a [reducer](/oss/graph-api#reducers) for the key you're updating in the parent graph state. See the example below.
</Warning>
