...
Welcome to LangChain! This quickstart will take you from zero to a fully
functional AI agent in just a few minutes. We'll start simple and gradually
build up to something more sophisticated.

## Super quick start

Let's begin with the absolute basics - creating a simple agent that can answer
questions and use tools:
...
Now let's create something more practical. We'll build a weather forecasting
agent that demonstrates the key concepts you'll use in production:
...
<Steps>
    ...
    <Step title="Create tools">
        :::python
        [Tools](/oss/langchain-tools) are functions your agent can call. They
        ...
        <Note>
            [Zod](https://zod.dev/) is a library for validating and parsing
            pre-defined schemas. You can use it to define the input schema for
            your tools to make sure the agent only calls the tool with the
            correct arguments.

            Alternatively, you can define the `schema` property as a JSON schema
            object. Keep in mind that JSON schemas *won't* be validated at
            runtime.
