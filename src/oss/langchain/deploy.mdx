...
Your application's code must reside in a GitHub repository to be deployed on LangGraph Platform. Both public and private repositories are supported. For this quickstart, first make sure your app is LangGraph-compatible by following the [local server setup guide](/oss/langchain/studio#setup-local-langgraph-server). Then, push your code to the repository.
...
2. Click the **LangGraph Studio** button in the top right corner. [LangGraph Studio](/oss/langchain/studio) will open to display your graph.
