...
Now we will start building components ([nodes](/oss/graph-api#nodes) and [edges](/oss/graph-api#edges)) for our agentic RAG graph.

:::python
Note that the components will operate on the [`MessagesState`](/oss/graph-api#messagesstate) — graph state that contains a `messages` key with a list of [chat messages](https://python.langchain.com/docs/concepts/messages/).
:::
