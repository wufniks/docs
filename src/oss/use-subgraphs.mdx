...
* [Shared state schemas](#shared-state-schemas) — parent and subgraph have **shared state keys** in their state [schemas](/oss/graph-api#state)
* [Different state schemas](#different-state-schemas) — **no shared state keys** in parent and subgraph [schemas](/oss/graph-api#state)
...
A common case is for the parent graph and subgraph to communicate over a shared state key (channel) in the [schema](/oss/graph-api#state). For example, in [multi-agent](/oss/multi-agent) systems, the agents often communicate over a shared [messages](/oss/graph-api#why-use-messages) key.
...
For more complex systems you might want to define subgraphs that have a **completely different schema** from the parent graph (no shared keys). For example, you might want to keep a private message history for each of the agents in a [multi-agent](/oss/multi-agent) system.
...
When you enable [persistence](/oss/persistence), you can [inspect the graph state](/oss/persistence#checkpoints) (checkpoint) via the appropriate method. To view the subgraph state, you can use the subgraphs option.
