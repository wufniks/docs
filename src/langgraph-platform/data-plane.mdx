...
The term "data plane" is used broadly to refer to [LangGraph Servers](langgraph-server) (deployments), the corresponding infrastructure for each server, and the "listener" application that continuously polls for updates from the [LangGraph Control Plane](control-plane).

## Server Infrastructure

In addition to the [LangGraph Server](langgraph-server) itself, the following infrastructure components for each server are also included in the broad definition of "data plane":
...
The data plane "listener" application periodically calls [control plane APIs](control-plane#control-plane-api) to:
...
<Info>
  **Only for Cloud SaaS**
  Data regions are only applicable for [Cloud SaaS](cloud) deployments.
</Info>
...
[`Production` type](control-plane#deployment-types) deployments automatically scale up to 10 containers. Scaling is based on 3 metrics:

1. CPU utilization
2. Memory utilization
3. Number of pending (in progress) [runs](assistants#execution)
...
<Info>
  **Only for Cloud SaaS**
  Static IP addresses are only available for [Cloud SaaS](cloud) deployments.
</Info>
...
<Info>
  Custom Postgres instances are only available for [Self-Hosted Data Plane](self-hosted-data-plane) and [Self-Hosted Control Plane](self-hosted-control-plane) deployments.
</Info>

A custom Postgres instance can be used instead of the [one automatically created by the control plane](control-plane#database-provisioning). Specify the [`POSTGRES_URI_CUSTOM`](env-var#postgres_uri_custom) environment variable to use a custom Postgres instance.
...
<Info>
  Custom Redis instances are only available for [Self-Hosted Data Plane](self-hosted-data-plane) and [Self-Hosted Control Plane](self-hosted-control-plane) deployments.
</Info>

A custom Redis instance can be used instead of the one automatically created by the control plane. Specify the [REDIS_URI_CUSTOM](env-var#redis_uri_custom) environment variable to use a custom Redis instance.
