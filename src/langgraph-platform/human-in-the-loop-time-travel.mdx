...
LangGraph provides the [**time travel**](/oss/time-travel) functionality to resume execution from a prior checkpoint, either replaying the same state or modifying it to explore alternatives. In all cases, resuming past execution produces a new fork in the history.
...
2. **Identify a checkpoint in an existing thread**: Use @[client.threads.get_history] method to retrieve the execution history for a specific `thread_id` and locate the desired `checkpoint_id`.
  Alternatively, set a [breakpoint](/oss/add-human-in-the-loop) before the node(s) where you want execution to pause. You can then find the most recent checkpoint recorded up to that breakpoint.
...
* [**LangGraph time travel guide**](/oss/time-travel): learn more about using time travel in LangGraph.
