...
Learn more about interrupts in the [How to handle interrupts](/oss/add-human-in-the-loop#pause-using-interrupt) guide.
...
* `onCustomEvent`: Called when a custom event is received. See the [streaming](/oss/use-streaming#stream-custom-data) guide to learn how to stream custom events.
