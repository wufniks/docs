...
* Manage [long term memory](/oss/memory)
* Debug agent state via [time travel](/oss/time-travel)
...
Chat mode is a simpler UI for iterating on and testing chat-specific agents. It is useful for business users and those who want to test overall agent behavior. Chat mode is only supported for graph's whose state includes or extends [`MessagesState`](/oss/use-graph-api#messagesstate).
