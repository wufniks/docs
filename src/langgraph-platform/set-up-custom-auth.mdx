...
2. [Make conversations private](resource-auth) - Let users have private conversations
3. [Connect an authentication provider](add-auth-server) - Add real user accounts and validate using OAuth2 for production
...
Now tell LangGraph to use authentication by adding the following to the [`langgraph.json`](cli#configuration-file) configuration:
...
You should now only be able to access the bot if you provide a valid token in the request header. Users will still, however, be able to access each other's resources until you add [resource authorization handlers](auth#resource-specific-handlers) in the next section of the tutorial.

![Authentication, no authorization handlers](images/authentication.png)
...
1. Continue the tutorial by going to [Make conversations private](resource-auth) to learn about resource authorization.
2. Read more about [authentication concepts](auth).
